...
public class LogWorkoutPage extends BasePage {
    ...
    public LogWorkoutPage createSessionSummary() {
        int randomHourStart = (int) (1 + Math.random()*24);
        int randomHourEnd = (int) (1 + Math.random()*24);
        int randomMinStart = (int) (1 + Math.random()*60);
        int randomMinEnd = (int) (1 + Math.random()*60);
    ...
    public String getBodyStatsValue(String fieldName) {
        String bodyStatusValue = driver.findElement(By.xpath(String.format(bodyStatsFieldValue, fieldName))).getText().replaceAll("[^0-9]", "");
        log.info("Return the text from the field - " + fieldName);
        return bodyStatusValue;
    }
    ...
    public LogWorkoutPage moveToTrainingStats() {
        new ButtonAHref(driver, "Training Stats").click();
